<?php
	...
	$team_stats = mysqli_query($db->link,"SELECT wins, losses 
	...
	$next_match = mysqli_query($db->link,"SELECT t.name,l.loc_name, c.date, c.time, t.wins, t.losses
	...
	$previous_matches= mysqli_query($db->link,"SELECT t.name,l.loc_name, c.date, 
				CASE
	...
	$team_name = mysqli_query($db->link,"SELECT t.name FROM team_members AS tm, users AS u, teams AS t WHERE t.team_id=tm.team_id AND tm.user_id=u.user_id AND u.username=('$user')");
	}

	//Select queries if another team has been 'selected'
	if(isset($other_team)){

	$last_match= mysqli_query($db->link,"SELECT t1.name, MAX(c.date) AS d, CASE
	...
	$other_stats=mysqli_query($db->link,"SELECT wins, losses FROM teams WHERE name=('$other_team')");

	$locations=mysqli_query($db->link,"SELECT loc_id, loc_name FROM locations");
	}
?>